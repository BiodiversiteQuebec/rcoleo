test_that("coleo_prep_input_data works as expected", {
  ...
  fake_pap_data <-
tibble::tribble(
          ~site_code,   ~type_hab, ~site_lat, ~site_lon,   ~opened_at,   ~closed_at,          ~type,       ~technician_1, ~temp_c, ~sky, ~wind,     ~category,       ~vernacular_fr,    ~rank,         ~taxa_name, ~abondance, ~tsn,
  ...
  expect_equal(ncol(nested_df$data[[1]]), 14)
