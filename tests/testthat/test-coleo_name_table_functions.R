test_that("coleo_name_table_functions", {
  expect_error(get_name_table_column("foo"))

  expect_equal(get_name_table_column("sites"),
               c("cell_id", "off_station_code_id", "site_code", "site_name",
                 "site_type", "site_opened_at", "site_lat", "site_lon", "site_notes"
               ))

  expect_equal(get_name_table_column("sites", column = "db_column"),
               c("cell_id", "off_station_code_id", "site_code", "site_name",
                 "type", "opened_at", "lat", "lon", "notes"))

  expect_error(get_name_table_column("sites", column = "foobar"), regexp = "column should.*")


  expect_equal(get_rename_vec_input_to_db("campaigns"),
