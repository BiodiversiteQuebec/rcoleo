...
However, because of the difference in the response for the taxa table, this might not always appear.
...
* is "rank" correct, and alwasy the same for the same tsn + name combination?
...
WAIT, now we want to make sure we are not injecting a species TWICE 

```{r}
species_inject_imaginary
```
