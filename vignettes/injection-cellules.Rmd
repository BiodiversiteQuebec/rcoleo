...
```r
cell_code <- c("124_93", "73_137") # Codes des nouvelles cellules à insérer
```
...
```r
shp_cells_list <- list() # Création d'une liste vide pour l'objet final

for (i in 1:length(cell_code)){

  shp <- shp_cells[shp_cells$IJ == cell_code[i] ,] # Obtention des information pour un cellule
  ...
  if(identical(cells_ls$name, character(0))){
    cells_ls$name <- NULL
  } # Information absente si pas de no mpour la cellule
  ...
  shp_cells_list[[i]] <- cells_ls # Alimentation de l'objet final avec un niveau pour chaque cellule
}
```
