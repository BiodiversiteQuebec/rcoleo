...
Nous devons procéder dans une boucle afin d'assurer une certain cohérence entre les tables observations et obs_soil_decomposition. 
...
for (row in 1:nrow(camp)) {
    this <- camp[row,]
    ## OBSERVATION
    obs_camp <- dplyr::select(this,
                              campaign_id,
                              date_obs,
                              is_valid
    )

    obs <- rcoleo::post_obs(apply(obs_camp, 1, as.list))
    observation_id <- obs[[1]]$body$id
    this$observation_id <- observation_id
    #Observation décomposition du sol
    if(!is.null(observation_id)){
      obs_spe_camp <- dplyr::select(this,
                                    observation_id,
                                    bag_no,
                                    type=type_bag,
                                    date_end=closed_at,
      ...
      resp<-rcoleo::post_obs_decomposition_sol(apply(obs_spe_camp, 1, as.list))
    }
}
