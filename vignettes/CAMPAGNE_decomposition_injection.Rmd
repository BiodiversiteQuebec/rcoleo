---
title: "Tutoriel pour injection des campagnes Décomposition"
...
|*type_hab*|type d'habitat du site (forestier, toundrique)|
|*technician_1*|Nom du technicienn(e)|
|*technician_2*|Nom du technicienn(e)|
|*type*|type d'habitat du site (forestier, toundrique)|
|*lat*|latitude du site d'échantillonnage|
|*lon*|longitude du site d'échantillonnage|
|*lon*|longitude du site d'échantillonnage|
|*type*|'thé vert' ou 'rooibos'|
|*bag_no*|Numéro du sac|
|*start_weight*|'Poids total du sac au début|
|*end_weight_with_bag*|Poids total à la fin|
|*end_weight_tea*|Poids du thé seulement à la fin|
|*shading*|'thé vert' ou 'rooibos|
|*human_impact*|'thé vert' ou 'rooibos|
|*opened_at*|date de début du prélèvement au format `YYYY-MM-DD`|
...
## 8. Injections des nouvelles campagnes de décomposition du sol. 
...
4. Observations des espèces ("*/obs_soil_decomposition*")
...
```r
# 8.1.1. Sélection
camp_camp <- dplyr::select(donnees_inj_cell_site_id,
                           site_id,
                           opened_at,
                           closed_at,
                           #technician_1,
...
camp_camp$type="décomposition_sol"
...
donnees_inj_cell_site_id$opened_at<-as.character(donnees_inj_cell_site_id$opened_at)
donnees_inj_cell_site_id$closed_at<-as.character(donnees_inj_cell_site_id$closed_at)
camp <- dplyr::left_join(donnees_inj_cell_site_id,
                         campaigns[, c("id", "site_id", "opened_at")],
                         by = c("site_id", "opened_at"))
                         
names(camp)[names(camp) == "id"] <- "campaign_id"
camp$type='décomposition_sol'
```
...
### 8.4. Injections des valeurs de décomposition du sol associées aux nouvelles campagnes

Les champs de la table **observations de décomposition du sol** de Coléo qui peuvent être remplis sont les suivants :

* *bag_no* : numéro du sachet
* *type* : 'thé vert','rooibos'
* *date_end* : date de la collecte du sachet
* *start_weight* : 
* *end_weight_with_bag* : 
* *end_weight_tea* : 
* *shading* : 
...
```r
# 8.4.3. Injection
COLEO_obsref_inj <- rcoleo::post_obs_decomposition_sol(obs_spe_camp_ls)
