...
|**sky**|'dégagé (0 à 10 %)', 'nuageux (50 à 90 %)', 'orageux', 'partiellement nuageux (10 à 50 %)', 'pluvieux'|
|**wind**|'calme (moins de 1 km/h)', 'très légère brise (1 à 5 km/h)', 'légère brise (6 à 11 km/h)', 'petite brise (12 à 19 km/h)', 'jolie brise (20 à 28 km/h)'|
...
## 8. Insertion des campagnes de papillons ####
...
```r
campagnes <- rcoleo::get_campaigns()
campagnes <- do.call(rbind.fill, campagnes[[1]]$body)
```
#### 8.1.7. On récupère le id unique pour chaque campagne nouvellement insérées.

```r
camp <- dplyr::left_join(camp, campagnes[, c("id", "site_id", "opened_at", "type")], by = c("site_id", "opened_at", "type"))
...
#### 8.5.6. Acquisition de la liste des observations de Coléo et mise à jour du tableau de données à injecter.
...
Bravo ! Vous êtes arrivés au bout de vos injections de nouvelles campagnes ! :sparkles:
